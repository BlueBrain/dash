...
{
    dash::NodePtr node = createSampleNode();

    NodeDist nodeDist( node );
    TEST( server->registerObject( &nodeDist ));

    NodeDist remoteNodeDist;
    TEST( client->mapObject( &remoteNodeDist, nodeDist.getID( )));

    TEST( areEqual( node, remoteNodeDist.getValue( )));

    client->unmapObject( &remoteNodeDist );
    server->deregisterObject( &nodeDist );
}
...
int dash::test::main( int argc, char **argv )
{
    ...
    testNodeSerialization( server, client );
